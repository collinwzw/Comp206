              ...
              unsigned char red, unsigned char green, unsigned char blue )
{
  ...
  if( open_return_code ){ printf( "bmp_open failed. Returning from bmp_mask without attempting changes.\n" ); return -1; }
    ...
    unsigned char(*data_array)[img_width*(bits_per_pixel/8)+padding] = malloc(img_height*sizeof*data_array);
    ...
    for(int i=img_height-1; i>=0;i--){ //row
        for (int j=0; j<(img_width) * (bits_per_pixel/8)+padding;j++){   //coloum
            ...
            if(j>=x_min*(bits_per_pixel/8) && j<=x_max* (bits_per_pixel/8) && i>=img_height-y_max  && i<= img_height-y_min){
                if (j%3==0)  data_array[i][j]=blue;
                if (j%3==1) data_array[i][j]=green;
                if (j%3==2) data_array[i][j]=red;
            }
            index=index+1;
        }
    }
...
int bmp_collage( char* bmp_input1, char* bmp_input2, char* bmp_result, int x_offset, int y_offset ){
  ...
  // YOUR CODE FOR Q3 SHOULD REPLACE EVERYTHING FROM HERE
    ...
    if (x_offset>=img_width1&& x_offset>=0){        // the img 2 will be right out side of the img 1 in terms of width
        new_width = (int) x_offset + img_width2 ;
    ...
    * new_width_p =   new_width;
    ...
    if (y_offset>=img_height1 && y_offset>=0 ){     // the new img is top out side of the img 1 in terms of height
        ...
        new_height = (int) img_height2 + y_offset ;
    }
    ...
    *new_datasize_p=  new_data_size;
    ...
    unsigned char(*data_array1)[img_width1*(bits_per_pixel1/8)] = malloc(img_height1*sizeof*data_array1);
    ...
    unsigned char(*data_array2)[img_width2*(bits_per_pixel2/8)] = malloc(img_height2*sizeof*data_array2);
     ...
     */
    unsigned char(*data_array)[new_width*(new_bpp/8)+newpadding] = malloc(new_height*sizeof*data_array);
    ...
    // caseindex assign
    if(x_offset>=0 && y_offset>=0) caseindex = 1;   // image 2 is width outside and height outside in postive offset direction.
    if (x_offset>=0 && y_offset<0) caseindex = 3;
    if(x_offset<0 && y_offset>=0) caseindex = 2;   // image 2 is width outside and height outside in postive offset direction.
    ...
    switch(caseindex){
            ...
            for(i=new_height-1; i>=0;i--){ //row
                ...
                for (j=0; j<(new_width) * (new_bpp/8)+newpadding;j++){   //coloum
                    ...
                    if (j>=((new_width) * (new_bpp/8))){        // if the array reaches the padding space if any.
                        ...
                        if (i<new_height-y_offset && i >=new_height-(y_offset+img_height2) && j >= ((x_offset)*(new_bpp/8)) && j< (x_offset + img_width2)*(new_bpp/8) ) {
                            data_array[i][j] = data_array2[itwo][jtwo];
                            jtwo++;
                            itwocount++;
                            if(i>=new_height - img_height1 && j <= (img_width1)*(new_bpp/8)){         // reaches the area of both image are present
                                jone++;
                            }
                            
                        }
                        else if (i >= new_height - y_offset && i >= new_height - img_height1 && j < img_width1 * (bits_per_pixel1/8) ){
                            data_array[i][j] = data_array1[ione][jone];
                            jone++;
                            
                        }
                        else if (i < new_height - (y_offset+ img_height2) && i >= new_height - img_height1 && j < img_width1 * (bits_per_pixel1/8) ){
                            data_array[i][j] = data_array1[ione][jone];
                            jone++;
                            
                        }
                        else if (i < new_height - y_offset  && i >= new_height - (y_offset+ img_height2)&&  ione>=0 && j <img_width1*(bits_per_pixel1/8)){
                            if (j < x_offset * (bits_per_pixel1/8) || j >= (x_offset+img_width2)*(new_bpp/8)){
                            data_array[i][j] = data_array1[ione][jone];
                            jone++;
                            }
                        }
                ...
                //printf("the outputfile %d th row's %d th element is  %d \n", i,j,data_array1[i][j]);

                
            }
            break;
            
        case 2:     //if(x_offset<0 && y_offset>=0)
            printf("case 2\n");
            //printf("checking %d th row", i);
            itwo=img_height2-1;
            jtwo=0;
            ...
            for(i=new_height-1; i>=0;i--){ //row
                ...
                for (j=0; j<(new_width) * (new_bpp/8)+newpadding;j++){   //coloum
                    ...
                    if (j>=((new_width) * (new_bpp/8))){        // if the array reaches the padding space if any.
                        ...
                        if ( i >=new_height-(img_height2+y_offset) && i<new_height-y_offset&& j < ((img_width2)*(new_bpp/8))) {   // print img 2
                            data_array[i][j] = data_array2[itwo][jtwo];
                            jtwo++;
                            itwocount++;
                        ...
                        else if (i >= new_height-img_height1 && j >= ((-x_offset)*(new_bpp/8)) && j<(-x_offset+img_width1)*(new_bpp/8) ){
                                data_array[i][j] = data_array1[ione][jone];
                                jone++;
                            
                        }
                        else if (i < new_height +y_offset  && i >= new_height-img_height1 && j >= ((-x_offset)*(new_bpp/8)) && j<(-x_offset+img_width1)*(new_bpp/8) ){
                            if (j>=img_width2*new_bpp/8){
                                data_array[i][j] = data_array1[ione][jone];
                                jone++;
                            }
                        }
            ...
            break;
        case 3:                     //if (x_offset>=0 && y_offset<0)
            printf("case 3\n");
            //printf("checking %d th row", i);
            itwo=img_height2-1;
            jtwo=0;
            ...
            for(i=new_height-1; i>=0;i--){ //row
                ...
                for (j=0; j<(new_width) * (new_bpp/8)+newpadding;j++){   //coloum
                    ...
                    if (j>=((new_width) * (new_bpp/8))){        // if the array reaches the padding space if any.
                        ...
                        if ( i >=new_height-img_height2 && j>=(x_offset)* (new_bpp/8) &&j < ((x_offset + img_width2)*(new_bpp/8))) {   // print img 2
                            data_array[i][j] = data_array2[itwo][jtwo];
                            jtwo++;
                            itwocount++;
                            if(i<new_height+y_offset ){         // reaches the area of both image are present
                                jone++;
                            }
                        }
                        else if (i < new_height+y_offset && j <img_width1 * new_bpp/8  ){
                            if(i < new_height-img_height2){
                                data_array[i][j] = data_array1[ione][jone];
                                jone++;
                                }
                            else if(j>= ((x_offset + img_width2)*(new_bpp/8)) || j<(x_offset)* (new_bpp/8)){
                                data_array[i][j] = data_array1[ione][jone];
                                jone++;
                                }
                        }
                        /*
                        else if (i < new_height +y_offset  && i >= new_height-img_height1 && j >= ((-x_offset)*(new_bpp/8))){   // blank area top side of img 2 if any
                            if (j>=img_width2*new_bpp/8){
                                data_array[i][j] = data_array1[ione][jone];
                                jone++;
                            }
        ...
        case 4:                 //if (x_offset <0 && y_offset<0)
            printf("case 4\n");
            ...
            for(i=new_height-1; i>=0;i--){ //row
                ...
                for (j=0; j<(new_width) * (new_bpp/8)+newpadding;j++){   //coloum
                    ...
                    if (j>=((new_width) * (new_bpp/8))){        // if the array reaches the padding space if any.
                        ...
                        else if (i < new_height-img_height2  && j >= ((-x_offset)*(new_bpp/8))&& j <((-x_offset+img_width1)*(new_bpp/8)) ){
                            if (i < new_height +y_offset){
                            data_array[i][j] = data_array1[ione][jone];
                            jone++;
                            }
                        }
                        else if (i < new_height +y_offset  && i >= new_height-img_height2 && j >= ((-x_offset)*(new_bpp/8))&& j <((-x_offset+img_width1)*(new_bpp/8))){
                            if (j>=img_width2*new_bpp/8){
                            data_array[i][j] = data_array1[ione][jone];
                            jone++;
                            }
